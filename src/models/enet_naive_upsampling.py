...
def encoder_build(inp, dropout_rate=0.01):
    ...
    # Bottleneck 1.0
    enet = encoder_bottleneck(enet, 64, name_prefix='en_bn_1.0', downsample=True, dropout_rate=dropout_rate)

    # Bottleneck 1.i
    for i in range(4):
        name_prefix = 'en_bn_1.{}'.format(i+1)
        enet = encoder_bottleneck(enet, 64, name_prefix, dropout_rate=dropout_rate)

    # Bottleneck 2.0
    enet = encoder_bottleneck(enet, 128, name_prefix='en_bn_2.0', downsample=True)
...
def decoder_build(encoder, nc):
    enet = decoder_bottleneck(encoder, 64, name_prefix='de_bn_4.0', upsample=True, reverse_module=True) # bottleneck 4.0
    enet = decoder_bottleneck(enet, 64, name_prefix='de_bn_4.1')                                        # bottleneck 4.1
    enet = decoder_bottleneck(enet, 64, name_prefix='de_bn_4.2')                                        # bottleneck 4.2
    enet = decoder_bottleneck(enet, 16, name_prefix='de_bn_5.0', upsample=True, reverse_module=True)    # bottleneck 5.0
    enet = decoder_bottleneck(enet, 16, name_prefix='de_bn_5.1')                                        # bottleneck 5.1
...
def get_model(input_shape, num_classes, encoder_only=False):
    ...
    if encoder_only:
        ...
        # dimension
        name = 'enet_naive_upsampling_encoder_only'
