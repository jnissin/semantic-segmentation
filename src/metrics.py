...
def semisupervised_accuracy(num_unlabeled, ignore_class=-1):
    ...
    def accuracy(y_true, y_pred):
        num_labeled = K.tf.cast(K.tf.shape(y_true)[0] - num_unlabeled, dtype=K.tf.int32)
        labels = K.tf.cast(K.tf.squeeze(y_true[0:num_labeled]), dtype=K.tf.int32)
        ...
        number_of_pixels = K.tf.ones_like(prediction_results, dtype=K.tf.float32) * K.tf.cast((K.tf.shape(predictions)[1] * K.tf.shape(predictions)[2]), dtype=K.tf.float32)
